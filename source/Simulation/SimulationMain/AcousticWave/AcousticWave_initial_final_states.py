...
# pre-allocating arrays (there are other ways to handle this, but this is the way I chose)
dens   = np.zeros(shape=(nState,nX))
pres   = np.zeros(shape=(nState,nX))
velx   = np.zeros(shape=(nState,nX))
eint   = np.zeros(shape=(nState,nX))
t      = np.zeros(nState)
fname  = []
...
for i in range(0,nState):
	with h5py.File(fname[i], "r") as hf:
		...
		# spatial coordinates are the same at beginning and end
		...
		dens[i,:]    = hf["dens"][0, 0, 0, :]
		pres[i,:]    = hf["pres"][0, 0, 0, :]
		velx[i,:]    = hf["velx"][0, 0, 0, :]
		eint[i,:]    = hf["eint"][0, 0, 0, :]
...
fs_label  = 14
fs_legend = 10
axs[0, 0].plot(x, dens[0,:], "r",  linewidth=3.0, label=r'$t=$'+'{:.2f}'.format(t[0])+' s')
axs[0, 0].plot(x, dens[1,:], "k:", linewidth=3.0, label=r'$t=$'+'{:.2f}'.format(t[1])+' s')
axs[0, 1].plot(x, pres[0,:], "r",  linewidth=3.0)
axs[0, 1].plot(x, pres[1,:], "k:", linewidth=3.0)
axs[1, 0].plot(x, velx[0,:], "r",  linewidth=3.0)
axs[1, 0].plot(x, velx[1,:], "k:", linewidth=3.0)
axs[1, 1].plot(x, eint[0,:], "r",  linewidth=3.0)
axs[1, 1].plot(x, eint[1,:], "k:", linewidth=3.0)
...
axs[1, 1].set_ylabel(r"$\epsilon_{\mathrm{int}}$", fontsize=fs_label)
