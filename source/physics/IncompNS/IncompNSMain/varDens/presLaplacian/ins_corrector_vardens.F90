...
subroutine ins_corrector_vardens(uni, vni, wni, sigx, sigy, sigz, pxn1, pyn1, pzn1, &
   ...
   do k = kz1, kz2
      do j = jy1, jy2
         do i = ix1, ix2 + 1
            Mdens = rhox(i, j, k)
            uni(i, j, k) = uni(i, j, k) - dt*Mdens*((p(i, j, k) - p(i - 1, j, k))/dx - sigx(i, j, k))
   ...
   do k = kz1, kz2
      do j = jy1, jy2 + 1
         do i = ix1, ix2
            Mdens = rhoy(i, j, k)
            vni(i, j, k) = vni(i, j, k) - dt*Mdens*((p(i, j, k) - p(i, j - 1, k))/dy - sigy(i, j, k))
   ...
   do k = kz1, kz2 + 1
      do j = jy1, jy2
         do i = ix1, ix2
            Mdens = rhoz(i, j, k)
            wni(i, j, k) = wni(i, j, k) - dt*Mdens*((p(i, j, k) - p(i, j, k - 1))/dz - sigz(i, j, k))
