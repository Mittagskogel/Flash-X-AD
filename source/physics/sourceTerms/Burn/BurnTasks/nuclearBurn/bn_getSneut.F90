!!****if* source/physics/sourceTerms/Burn/BurnTasks/nuclearBurn/bn_getSneut
!! NOTICE
...
!! ARGUMENTS
!!
!!   azBarEtc - state information for the current cell
!!
!! DESCRIPTION
!!
...
!!
!!  NOTES
!!    Calls function bn_ifermi12
!!
!!***

function bn_getSneut(azbarEtc) result(sneut)
...
!  use Burn_dataEOS, ONLY:  btemp,bden,abar,zbar,z2bar,ytot1,bye
  ...
  TYPE(bn_azbarEtc_t),intent(IN) :: azbarEtc
  real ::    sneut
...
!!    declare local variables
  ...
  real :: bye           ! We could use azbarEtc % bye instead. - KW
...
!!    propagate through the routine. cv and ca are the vector and axial currents.
  ...
  ASSOCIATE(abar    => azbarEtc % abar,     &
            zbar    => azbarEtc % zbar,     &
            bden    => azbarEtc % bden,     &
            btemp   => azbarEtc % btemp)
...
!!    some frequent factors
  xmue  = abar / zbar
  bye   = zbar / abar           ! We could use azbarEtc % bye here instead. - KW
  t9    = btemp * 1.0e-9
